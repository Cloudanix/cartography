...
def collect_critical_relationships(driver) -> Dict[str, int]:
    ...
    checks = {
        "AWSAccount->EC2Instance (RESOURCE)": "MATCH (:AWSAccount)-[:RESOURCE]->(:EC2Instance) RETURN count(*) AS cnt",
        "AWSAccount->AWSRole (AWS_ROLE)": "MATCH (:AWSAccount)-[:AWS_ROLE]->(:AWSRole) RETURN count(*) AS cnt",
        "AWSAccount->S3Bucket (RESOURCE)": "MATCH (:AWSAccount)-[:RESOURCE]->(:S3Bucket) RETURN count(*) AS cnt",
        "GCPProject->GCPInstance (RESOURCE)": "MATCH (:GCPProject)-[:RESOURCE]->(:GCPInstance) RETURN count(*) AS cnt",
        "AzureTenant->AzureSubscription (RESOURCE)": "MATCH (:AzureTenant)-[:RESOURCE]->(:AzureSubscription) RETURN count(*) AS cnt",
    }
    results = {}
...
def compare(
    before: Dict, after: Dict, tolerance: float, report: ComparisonReport
) -> None:
...
def print_report(report: ComparisonReport) -> None:
    ...
    for severity in [CRITICAL, WARNING, INFO]:
        ...
        for d in items:
            ...
            wrapped = textwrap.fill(
                d.recommendation, width=74, subsequent_indent="            "
            )
            print(wrapped)
    ...
    if critical > 0:
        print(
            f"  RESULT: FAIL — {critical} critical issue(s) require immediate attention."
        )
        print(
            f"  DO NOT promote this build to production until critical items are resolved."
        )
    elif report.summary.get(WARNING, 0) > 0:
        print(
            f"  RESULT: WARN — {total} deviation(s) found. Review warnings before promoting."
        )
    else:
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    # Snapshot mode
    p.add_argument(
        "--snapshot-save", help="URI to snapshot from (saves to --snapshot-file)"
    )
    p.add_argument(
        "--snapshot-load", help="Path to load a pre-saved snapshot as 'before' data"
    )
    p.add_argument(
        "--snapshot-file",
        default="cartography_snapshot.json",
    ...
    p.add_argument(
        "--output", help="Write JSON report to this file in addition to stdout"
    )
