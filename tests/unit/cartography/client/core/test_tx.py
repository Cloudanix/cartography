...
def _client_error_with_code(message, code):
    """neo4j driver >=6 makes ClientError.code a read-only property; build a
    subclass instance whose code returns the desired value."""
    err_cls = type("_FakeClientError", (ClientError,), {"code": property(lambda self: code)})
    return err_cls(message)


def entity_not_found_error():
    err = _client_error_with_code("entity gone", "Neo.ClientError.Statement.EntityNotFound")
    return err
...
def test_ensure_indexes_ignores_parallel_index_creation_race(mock_build):
    ...
    race = _client_error_with_code("already exists", "Neo.ClientError.Schema.EquivalentSchemaRuleAlreadyExists")
    session = MagicMock()
...
def test_ensure_indexes_raises_other_client_errors(mock_build):
    ...
    err = _client_error_with_code("denied", "Neo.ClientError.Security.Forbidden")
    session = MagicMock()
...
def test_load_graph_data_default_batch_size_is_500():
    session = MagicMock()
    data = [{"id": i} for i in range(501)]
...
class TestRetryClassification:
    ...
    def test_other_client_error_is_not_retryable(self):
        err = _client_error_with_code("bad", "Neo.ClientError.Statement.SyntaxError")
...
def test_run_index_query_ignores_equivalent_schema_rule_already_exists(mock_logger):
    ...
    """
    mock_session = MagicMock()
    mock_session.execute_write.side_effect = _create_client_error(
...
def test_run_index_query_succeeds_normally():
    ...
    mock_session.execute_write.assert_called_once_with(
        write_query_tx, "CREATE INDEX IF NOT EXISTS ...",
    )
