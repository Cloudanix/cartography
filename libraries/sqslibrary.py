...
class SQSLibrary:
        ...
        try:
            ...
            if error_code == "InvalidParameterValue":
                self.context.logger.debug(
                    "InvalidParameterValue - Failed to delete message in SQS",
                    exc_info=True,
                    stack_info=True,
                    extra={
                ...
                self.context.logger.debug(
                    f"The message receipt handle has expired - {e}"
                )

            else:
                self.context.logger.error(
                    "Failed to delete message in SQS",
                    exc_info=True,
                    stack_info=True,
                    extra={
                        "message_receipt_handle": message_receipt_handle,
    ...
    def change_message_visibility(
        self, message_receipt_handle: str, duration: int
    ) -> bool:
        try:
            ...
            if error_code == "InvalidParameterValue":
                self.context.logger.debug(
                    "InvalidParameterValue - Failed to extend visibility timeout for message in SQS",
                    exc_info=True,
                    stack_info=True,
                    extra={
                        "message_receipt_handle": message_receipt_handle,
                ...
                self.context.logger.debug(
                    f"Adjusted visibility timeout to: {visibility_timeout} - {e}"
                )

            else:
                # INFO: ignore this error as the message might have been processed by another worker already
                self.context.logger.debug(
                    "Failed to extend visibility timeout for message in SQS",
                    extra={
                        "message_receipt_handle": message_receipt_handle,
                        "timeout": duration,
                        "error": str(e),
    ...
    def publish(self, message: dict, topic: str, attributes: dict = {}) -> bool:
        ...
        try:
            ...
            response = self.sqs_client.send_message(
                ...
                MessageGroupId=str(
                    uuid.uuid4()
                ),  # Each message gets a unique group_id as we need to process all messages in parallel
                MessageAttributes=custom_attributes,
            )

            self.context.logger.info(
                "Message published successfully to SQS Queue",
                extra={
