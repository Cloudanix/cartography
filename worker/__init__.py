...
def process_request(msg: Dict):
    logger.info(
        f'{msg["templateType"]} request received - {msg["eventId"]} - {msg["workspace"]}'
    )
...
def main(event: func.EventGridEvent, outputEvent: func.Out[func.EventGridOutputEvent]):
    ...
    try:
        ...
        if resp.get("status") == "success":
            logger.info(
                f'successfully processed cartography: {msg["eventId"]} - {json.dumps(resp)}'
            )

        else:
            logger.info(
                f'failed to process cartography: {msg["eventId"]} - {resp["message"]}'
            )
        ...
        if resp.get("services", None):
            if message.get("requestTopic"):
                ...
                access_key = os.environ.get(
                    "CDX_AZURE_CARTOGRAPHY_REQUEST_TOPIC_ACCESS_KEY"
                )
        ...
        logger.error(
            f"failed to process request from event grid: {ex}",
            exc_info=True,
            stack_info=True,
        )
