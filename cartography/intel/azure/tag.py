...
@timeit
def get_resource_groups_list(
    client: ResourceManagementClient, common_job_parameters: Dict
) -> List[Dict]:
    try:
        resource_groups_list = list(
            map(lambda x: x.as_dict(), client.resource_groups.list())
        )
...
def _load_resource_groups_tx(
    ...
    subscription_id: str,
    resource_groups_list: List[Dict],
    update_tag: int,
) -> None:
    ingest_group = """
...
def cleanup_resource_groups(
    ...
    run_cleanup_job(
        "azure_import_resource_groups_cleanup.json",
        neo4j_session,
...
def cleanup_tags(neo4j_session: neo4j.Session, common_job_parameters: Dict) -> None:
    run_cleanup_job(
        "azure_import_tags_cleanup.json", neo4j_session, common_job_parameters
    )
...
def sync_tags(
    ...
    get_tags_list(
        config, client, resource_groups_list, update_tag, common_job_parameters
    )
...
def sync_resource_groups(
    ...
    load_resource_groups(
        neo4j_session, subscription_id, resource_groups_list, update_tag
    )
