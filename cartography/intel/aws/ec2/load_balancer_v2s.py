...
def _safe_console_link(arn: str) -> str:
    """Console links are best-effort; malformed/empty ARNs must not fail the sync."""
    if not aws_console_link or not arn:
        return ""
    try:
        ...
        lb_data.append(
            {
                ...
                "consolelink": _safe_console_link(lb.get("LoadBalancerArn", "")),
            }
        )
