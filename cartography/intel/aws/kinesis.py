...
def get_kinesis_streams(boto3_session: boto3.session.Session, region: str) -> List[Dict]:
    ...
    try:
        ...
        for name in names:
            try:
                summary = client.describe_stream_summary(StreamName=name).get('StreamDescriptionSummary', {})
            except ClientError as e:
                logger.warning(f"Failed to describe kinesis stream {name} - {e}")
                summary = {}
            arn = summary.get('StreamARN') or f"arn:aws:kinesis:{region}::stream/{name}"
            streams.append({
                'arn': arn,
                'name': name,
                'region': region,
        ...
        logger.warning(f"Failed to call kinesis list_streams: {region} - {e}")
    return streams
...
def sync(
    ...
    for region in regions:
        streams.extend(get_kinesis_streams(boto3_session, region))
    load_kinesis_streams(neo4j_session, streams, current_aws_account_id, update_tag)
