...
def get_dataflow_jobs(
    dataflow: Resource, project_id: str, regions: list, common_job_parameters
) -> List[Dict]:
    jobs = []
    try:
        ...
        while req is not None:
            ...
            req = (
                dataflow.projects()
                .jobs()
                .aggregated_next(previous_request=req, previous_response=res)
            )
        return jobs
...
@timeit
def cleanup_dataflow_jobs(
    neo4j_session: neo4j.Session, common_job_parameters: Dict
) -> None:
    run_cleanup_job(
        "gcp_dataflow_jobs_cleanup.json", neo4j_session, common_job_parameters
    )
