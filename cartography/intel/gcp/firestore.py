...
@timeit
def get_firestore_databases(
    firestore: Resource, project_id: str, regions: list, common_job_parameters
) -> List[Dict]:
    """
        ...
        if response.get("databases", []):
            for database in response["databases"]:
                ...
                database["consolelink"] = gcp_console_link.get_console_link(
                    resource_name="firestore_collection",
                    project_id=project_id,
                    firestore_collection_name=database["name"].split("/")[-1],
                )
                if regions is None or len(regions) == 0:
                    firestore_databases.append(database)
                else:
                    if (
                        database["locationId"] in regions
                        or database["locationId"] == "global"
                    ):
                        firestore_databases.append(database)
...
@timeit
def get_firestore_indexes(
    firestore: Resource, database: Dict, project_id: str
) -> List[Dict]:
    """
...
def transform_indexes(
    indexes: List[Dict], database: Dict, project_id: str
) -> List[Dict]:
    firestore_indexes = []
    for index in indexes:
        ...
        index["consolelink"] = gcp_console_link.get_console_link(
            project_id=project_id, resource_name="firestore_index"
        )
...
def sync(
    ...
    load_firestore_databases(
        neo4j_session, firestore_databases, project_id, gcp_update_tag
    )
    label.sync_labels(
        neo4j_session,
        firestore_databases,
    ...
    for database in firestore_databases:
        ...
        load_firestore_indexes(
            neo4j_session, firestore_indexes, project_id, gcp_update_tag
        )
