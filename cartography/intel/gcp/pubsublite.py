...
@timeit
def get_pubsublite_topics(
    pubsublite: Resource, project_id: str, regions: list, common_job_parameters
) -> List[Dict]:
    topics = []
    try:
        req = (
...
@timeit
def cleanup_pubsublite_topics(
    neo4j_session: neo4j.Session, common_job_parameters: Dict
) -> None:
    run_cleanup_job(
        "gcp_pubsublite_topics_cleanup.json", neo4j_session, common_job_parameters
    )


@timeit
def get_pubsublite_subscriptions(
    pubsublite: Resource, project_id: str, regions: list, common_job_parameters
) -> List[Dict]:
    subscriptions = []
    try:
...
@timeit
def transform_pubsublite_subscriptions(
    subscriptions: List[Dict], project_id: str
) -> List[Dict]:
    list_subscriptions = []
...
@timeit
def load_pubsublite_subscriptions(
    session: neo4j.Session, data_list: List[Dict], project_id: str, update_tag: int
) -> None:
    session.write_transaction(
        load_pubsublite_subscriptions_tx, data_list, project_id, update_tag
    )


@timeit
def load_pubsublite_subscriptions_tx(
    tx: neo4j.Transaction, data: List[Dict], project_id: str, gcp_update_tag: int
) -> None:
    query = """
    UNWIND $Records as record
...
@timeit
def cleanup_pubsublite_subscriptions(
    neo4j_session: neo4j.Session, common_job_parameters: Dict
) -> None:
    run_cleanup_job(
        "gcp_pubsublite_subscriptions_cleanup.json",
        neo4j_session,
...
def sync(
    ...
    topics = get_pubsublite_topics(
        pubsublite, project_id, regions, common_job_parameters
    )
    transformed_topics = transform_pubsublite_topics(topics, project_id)
    load_pubsublite_topics(
        neo4j_session, transformed_topics, project_id, gcp_update_tag
    )

    subscriptions = get_pubsublite_subscriptions(
        pubsublite, project_id, regions, common_job_parameters
    )
    transformed_subscriptions = transform_pubsublite_subscriptions(
        subscriptions, project_id
    )
    load_pubsublite_subscriptions(
        neo4j_session, transformed_subscriptions, project_id, gcp_update_tag
    )
