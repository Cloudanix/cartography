...
@timeit
def get_dataproc_clusters(
    dataproc: Resource, project_id: str, regions: list
) -> List[Dict]:
    clusters = []
    try:
        if regions:
            for region in regions:
                for state_filter in DATAPROC_LIST_STATE_FILTERS:
                    req = (
                        ...
                        .regions()
                        .clusters()
                        .list(projectId=project_id, region=region, filter=state_filter)
                    )
                    while req is not None:
                        ...
                        if res.get("clusters"):
                            for cluster in res["clusters"]:
                                ...
                                cluster["id"] = (
                                    f"projects/{project_id}/clusters/{cluster['clusterName']}"
                                )
                                cluster["consolelink"] = (
                                    gcp_console_link.get_console_link(
                                        project_id=project_id,
                                        dataproc_cluster_name=cluster["clusterName"],
...
@timeit
def cleanup_dataproc_clusters(
    neo4j_session: neo4j.Session, common_job_parameters: Dict
) -> None:
    run_cleanup_job(
        "gcp_dataproc_clusters_cleanup.json", neo4j_session, common_job_parameters
    )
