            ...
            while request is not None:
                ...
                if response.get('keyRings', []):
                    for key_ring in response['keyRings']:
                        ...
                        key_ring['members'] = key_ring.get('iam_policy',{}).get('bindings',[]).get('members',[])
                        for user in key_ring['members']:
                            key_ring['public_policy'] = False
            ...
            while request is not None:
                ...
                if response.get('cryptoKeys', []):
                    for crypto_key in response['cryptoKeys']:
                        ...
                        crypto_key['members'] = crypto_key.get('iam_policy',{}).get('bindings',[]).get('members',[])
                        for user in crypto_key['members']:
