...
@timeit
def get_apigateway_locations(
    apigateway: Resource, project_id: str, common_job_parameters
) -> List[Dict]:
    """
...
def transform_locations(locations: List[Dict], project_id: str) -> List[Dict]:
    ...
    for location in locations:
        ...
        location["consolelink"] = gcp_console_link.get_console_link(
            project_id=project_id, resource_name="apigateway_home"
        )
...
@timeit
def get_apis(
    apigateway: Resource, project_id: str, regions: list, common_job_parameters
) -> List[Dict]:
    """
            ...
            while req is not None:
                ...
                if res.get("apis", []):
                    for api in res["apis"]:
                        ...
                        api_entities, public_access = get_api_policy_entities(
                            apigateway, api, project_id
                        )
                        ...
                        api["consolelink"] = gcp_console_link.get_console_link(
                            ...
                            managed_service_name=api["managedService"],
                            api_name=api["displayName"],
                        )
                        apis.append(api)
                req = (
                    apigateway.projects()
...
@timeit
def get_api_policy_entities(
    apigateway: Resource, api: Dict, project_id: str
) -> List[Dict]:
    """
...
@timeit
def get_api_configs(
    apigateway: Resource, project_id: str, api: Dict, regions: list
) -> List[Dict]:
    """
            ...
            if res.get("apiConfigs", []):
                for apiConfig in res["apiConfigs"]:
                    ...
                    apiConfig["consolelink"] = gcp_console_link.get_console_link(
                        resource_name="api_config",
                        project_id=project_id,
                        managed_service_name=api["managedService"],
                        api_name=api.get("name").split("/")[-1],
                        api_config_name=apiConfig.get("name").split("/")[-1],
                        api_configuration_id=apiConfig["serviceConfigId"],
                    )
            ...
            while req is not None:
                ...
                if res.get("gateways", []):
                    for gateway in res["gateways"]:
                        ...
                        gateway_entities, public_access = get_api_policy_entities(
                            apigateway, gateway, project_id
                        )
                        ...
                        gateway["consolelink"] = gcp_console_link.get_console_link(
                            ...
                            api_gateway_name=gateway.get("name").split("/")[-1],
                        )
...
@timeit
def get_api_gateway_policy_entities(
    apigateway: Resource, gateway: Dict, project_id: str
) -> List[Dict]:
    """
...
@timeit
def cleanup_apigateway_locations(
    ...
    """
    run_cleanup_job(
        "gcp_apigateway_locations_cleanup.json", neo4j_session, common_job_parameters
    )
...
@timeit
def load_apis_tx(
    tx: neo4j.Transaction, apis: List[Dict], project_id: str, gcp_update_tag: int
) -> None:
    """
...
def cleanup_apis(neo4j_session: neo4j.Session, common_job_parameters: Dict) -> None:
    ...
    """
    run_cleanup_job(
        "gcp_apigateway_apis_cleanup.json", neo4j_session, common_job_parameters
    )
...
@timeit
def load_apis_entity_relation_tx(
    tx: neo4j.Transaction, api: Dict, gcp_update_tag: int
) -> None:
    """
    :type neo4j_session: Neo4j session object
    :param neo4j session: The Neo4j session object
...
@timeit
def load_gateway_entity_relation_tx(
    tx: neo4j.Transaction, gateway: Dict, gcp_update_tag: int
) -> None:
    """
    :type neo4j_session: Neo4j session object
    :param neo4j session: The Neo4j session object
...
@timeit
def load_api_configs_tx(
    tx: neo4j.Transaction, configs: List[Dict], project_id: str, gcp_update_tag: int
) -> None:
    """
...
def cleanup_api_configs(
    ...
    """
    run_cleanup_job(
        "gcp_apigateway_configs_cleanup.json", neo4j_session, common_job_parameters
    )
...
@timeit
def load_gateways_tx(
    tx: neo4j.Transaction, gateways: List[Dict], project_id: str, gcp_update_tag: int
) -> None:
    """
...
def cleanup_api_gateways(
    ...
    """
    run_cleanup_job(
        "gcp_apigateway_gateways_cleanup.json", neo4j_session, common_job_parameters
    )
