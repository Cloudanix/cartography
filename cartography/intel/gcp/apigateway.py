            ...
            if res.get('apis', []):
                for api in res['apis']:
                    ...
                    api['iam_policy'] = apigateway.projects().locations().apis().getIamPolicy(\
                        resource=f"projects/{project_id}/locations/global/apis/{api['name']}").execute()
                    api['members'] = api.get('iam_policy',{}).get('bindings',[]).get('members',[])
                    for user in api['members']:
                        api['public_api_policy'] = False
                        if user == 'allUsers' or user == 'allAuthenticatedUsers':
                            api['public_api_policy'] = True
                            break
                    apis.append(api)
        ...
        while req is not None:
            ...
            if res.get('apiConfigs', []):
                for apiConfig in res['apiConfigs']:
                    ...
                    apiConfig['members'] = apiConfig.get('iam_policy',{}).get('bindings',[]).get('members',[])
                    for user in apiConfig['members']:
                        apiConfig['public_config_policy'] = False
                        if user == 'allUsers' or user == 'allAuthenticatedUsers':
                            apiConfig['public_config_policy'] = True
                            break
            ...
            if res.get('gateways', []):
                for gateway in res['gateways']:
                    ...
                    gateway['iam_policy'] = apigateway.projects().locations().gateways().getIamPolicy(\
                        resource = f"projects/{project_id}/locations/global/gateways/{gateway['name']}")
                    gateway['members'] = gateway.get['iam_policy',{}].get['bindings',[]].get('members',[])
                    for user in gateway['members']:
                        gateway['public_gateway_policy'] = False
                        if user == 'allUsers' or user == 'allAuthenticatedUsers':
